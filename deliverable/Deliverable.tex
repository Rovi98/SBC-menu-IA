...
\begin{itemize}
	...
	\item \textbf{Formalización}, donde partiendo de los conceptos de la fase anterior, procederemos a su formalización desarrollando una ontología. También, para la resolución del problema formalizaremos el conocimiento y una metodología de resolución.
...
Por ejemplo, si una persona es vegetariana y además diabética, se le generaría un menú sin carne y con poca cantidad de azúcares. Además tal y como hemos mencionado, nuestro sistema también admite preferencias, así pues a una persona que tenga osteoporosis y no le guste nada la verdura se le intentará asignar un menú que contenga calcio y que no tenga platos con verduras. 
...
\begin{itemize}
	\item \textbf{El usuario}: la persona que utilice nuestro sistema tendrá que indicar e introducir información acerca de su edad, sexo, enfermedades...para que podamos proporcionarle el mejor plan alimenticio. 
...
\item \textbf{Persona}: las características necesarias para definir correctamente a la persona (usuario) son: \begin{itemize}
	...
	\item \textbf{Preferencias alimenticias}: tales como el ser vegetariano, vegano, o practicar alguna religión con requerimientos alimenticios específicos (como el islam o el judaísmo).
	\end{itemize}
	...
	\item \textbf{Ingredientes}: las características necesarias para definir correctamente a un ingrediente son las siguientes: \begin{itemize}
...
\item \textbf{Enfermedades}: cada enfermedad, además de su nombre la cual nos sirve para definirla en sí, tenemos para cada enfermedad el conocimiento de qué limitaciones alimenticias presenta. \begin{itemize}
	\item \textbf{Nombre}
...
Para decidir que platos entran en el menú primero de todo vamos a coger los 7 desayunos y los 15 postres mejor puntuados del apartado anterior y los vamos a fijar en nuestro menú. A continuación fijaremos todos los primeros platos menos 4 y todos los segundos platos menos 4 (cogiendo los que tengan mejor puntuación).

Finalmente con los platos que falten, probaremos distintas posibilidades (siendo más flexibles en cuanto a las preferencias de la persona) para tratar de obtener un menu que se adecúe a las necesidades energéticas del individuo.
...
\begin{itemize}
	\item En el caso de los vegetarianos vamos a penalizar los platos que tengan algún ingrediente de carne.
...
Primero ordenaremos los platos según si se adecúan o no a las necesidades y preferencias de la persona (las necesidades médicas se tratarán de forma más estricta que las preferencias).

Descartaremos los platos peor puntuados de la lista, ya que perjudicarían el menú.

Con los platos restantes diseñaremos el menú más adecuado para cada usuario. 
...
\begin{itemize}
	...
	\item \textbf{Módulo printing}: la función de este módulo es imprimir la información al usuario y finalizar la ejecución.
\end{itemize}
...
Una vez implementado nuestro SBC, hemos realizado una serie de pruebas enfocadas a testear diferentes puntos de la aplicación. Así pues, mediante estas pruebas, hemos podido comprobar el correcto funcionamento del sistema, tanto a nivel general como a nivel particular.
...
En este apartado hemos obtenido un resultado con pocas calorías. Esto es debido a que la persona realiza mucho ejercicio, esto provoca que la cantidad de calorías al día sea muy grande. Por otro lado al ser musulmán, no puede comer cerdo, por lo tanto todos los platos con bastantes calorías que contienen cerdo no serán cogidos.
Esto se podría solucionar si el programa tuviese mas variedad de platos con mayores cantidades para personas que no comen carne.

\subsection{Prueba 4: Mujer con gota que necesita pocas calorías con preferencias alimentícias}

En este caso hemos querido probar como funcionaba nuestro SBC con una \textbf{mujer con gota y que necesita pocas calorías}. La mujer en cuestión además tiene \textbf{preferencias alimenticias que no están acorde con la enfermedad}. Es decir, prefiere comer carne y marisco cuando su enfermedad se lo restringe. 
...
\subsubsection{Solución esperada}
Esperamos obtener una solución que esté acorde con los requisitos alimenticios de la gota. Es importante mencionar que las preferencias de la persona son contrarias a las restricciones de la enfermedad. No obstante, debido a que las restricciones de enfermedad tienen más peso que las otras, esperamos seguir obteniendo un buen resultado. Además también queremos probar como se comporta nuestro sistema cuando el individuo necesita una cantidad muy pequeña de calorías. 

\subsubsection{Resultado obtenido}

En este caso hemos obtenido un resultado bastante curioso, porque al no poder comer carne, pero al gustarle mucho, aparece bastantes veces la carne en los resultados. Esto provoca que seguramente el menú obtenido no seria del todo adecuado para esta persona, porque hay demasiado carne para lo que su enfermedad requiere.
...
Hemos obtenido así un menú que se acerca mucho a las necesidades calóricas del individuo, obteniendo un menú con valor calórico medio-bajo.
...
\appendixpage
\addappheadtotoc
\section{Juegos de prueba completos}
En este anexo incluiremos los \emph{intputs} y \emph{outputs} completos de los casos de prueba que antes hemos tratado al ejecutarlos en nuestro sistema \emph{CLIPS}.
